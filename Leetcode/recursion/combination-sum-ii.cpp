...
class Solution
{
    ...
    void findSubsets(vector<int> &nums, vector<vector<bool>> &dp, const int &R, const int &C, int t, int i, int target, vector<int> &resSoFar, set<vector<int>> &result)
    {
        ...
        if (t <= 0 || i <= 0 && target == 0)
        {
            vector<int> resSoFarCpy(resSoFar.begin(), resSoFar.end());
            sort(resSoFarCpy.begin(), resSoFarCpy.end());
            result.insert(resSoFarCpy);
            return;
        }
        ...
        if (t >= nums[i - 1] && i >= 0 && dp[t - nums[i - 1]][i])
        {
            ...
            findSubsets(nums, dp, R, C, t - nums[i - 1], i - 1, target - nums[i - 1], resSoFar, result);
            resSoFar.pop_back();
        }
        if (i - 1 >= 0 && dp[t][i - 1])
        {
