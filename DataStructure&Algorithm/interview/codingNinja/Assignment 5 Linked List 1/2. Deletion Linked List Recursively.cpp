...
void Dhelper(node* p,node* q,int k,int position){
	if(k<position && p!=NULL){
		q=p;
		p=p->next;
		Dhelper(p,q,k+1,position);
	}else{
		q->next=p->next;
		p->next=NULL;
		delete p;
...
node* delete_node_recursion(node* head,int pos){
    pos+=1;
    int k=1;
	node* p=head;
	node* q=NULL;
...
int main(){
	...
	print(head);
	head=delete_node_recursion(head,0);
	print(head);
	head=delete_node_recursion(head,3);
	print(head);
	head=delete_node_recursion(head,1);
	print(head);
}
